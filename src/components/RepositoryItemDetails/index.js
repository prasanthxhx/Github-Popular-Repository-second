...
class RepositoryItemDetails extends Component {
  ...
  renderSuccessView = () => {
    ...
    const colors = formatedLanguages.map(() => generateRandomColor())
      ...
      <div className="repos-item-cont repos-details-card">
        ...
        <div className="languages-cont">
          {formatedLanguages.map(eachItem => (
            <p className={`languages ${getColorClassName()}`} key={eachItem.id}>
              {eachItem.name}
            </p>
          ))}
        </div>
        ...
        >
          <PieChart className="piechart">
            <Pie
              ...
              dataKey="value"
            >
              {formatedLanguages.map((entry, index) => (
                <Cell
                  key={entry.id}
                  name={entry.name}
